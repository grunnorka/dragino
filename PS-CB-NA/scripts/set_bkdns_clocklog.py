#!/usr/bin/env python3
"""Set backup DNS and CLOCKLOG after the device is fully attached and idle."""
...
def main() -> int:
    ...
    print(f"Port={args.port} setting BKDNS={BKDNS} CLOCKLOG={CLOCKLOG}", flush=True)
    ...
    for attempt in range(10):
        try:
            ...
            if not unlock():
                ...
                time.sleep(1.0)
                continue
            print("Setting BKDNS and CLOCKLOG...", flush=True)
            send(f"AT+BKDNS={BKDNS}", 2.0)
            time.sleep(1.0)
            send("AT+BKDNS=?", 2.5)
            send(f"AT+CLOCKLOG={CLOCKLOG}", 2.0)
            time.sleep(1.0)
            send("AT+CLOCKLOG=?", 2.5)
            send("AT+CFG", 2.5)
            send("ATZ", 2.0)
            ser.close()
