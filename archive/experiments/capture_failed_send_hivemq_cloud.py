#!/usr/bin/env python3
"""Compare Failed-to-send on HiveMQ Cloud (TLS 8883) vs prior private TB."""
...
HIVEMQ_HOST = "e148794f16884ca5b5524102f9efc7f2.s1.eu.hivemq.cloud"
HIVEMQ_PORT = 8883
HIVEMQ_USER = "dragino"
HIVEMQ_PASS = "dragino123"
...
def main() -> None:
    ...
    logpath = ROOT / "logs" / f"{stamp}_failed_send_hivemq_cloud.raw.log"
    ...
    def apply_hivemq() -> None:
        """MQTT JSON + HiveMQ Cloud TLS. No ATZ (avoid HiveMQ public rewrite)."""
        # Prefer JSON MQTT over ThingsBoard profile for this broker
        send("AT+PRO=3,5", 2.5)
        # TLS enable, no mutual auth (HiveMQ Cloud user/pass)
        send("AT+TLSMOD=1,0", 2.0)
        send(f"AT+SERVADDR={HIVEMQ_HOST},{HIVEMQ_PORT}", 2.5)
        send("AT+BKDNS=1,0", 2.0)
        send(f"AT+UNAME={HIVEMQ_USER}", 1.5)
        send(f"AT+PWD={HIVEMQ_PASS}", 1.5)
    ...
    apply_hivemq()
    # Re-assert SERVADDR after PRO (PRO may rewrite defaults)
    unlock()
    send(f"AT+SERVADDR={HIVEMQ_HOST},{HIVEMQ_PORT}", 2.5)
    send("AT+BKDNS=1,0", 2.0)
    send("AT+TLSMOD=1,0", 2.0)
    ...
    log("TEST", f"LISTEN {TARGET_CYCLES} HiveMQ Cloud cycles, up to {LISTEN_S}s")
