...
from dashboard.extract import extract_common, resolve_device_id
...
def main() -> int:
    ...
    def on_message(_c: mqtt.Client, _u: object, msg: mqtt.MQTTMessage) -> None:
        ...
        device_id = resolve_device_id(topic, extracts)
        if not device_id:
        ...
        try:
            ...
            log.info(
                "uplink device=%s imei=%s battery=%s signal=%s model=%s",
                device_id,
                extracts.get("imei"),
